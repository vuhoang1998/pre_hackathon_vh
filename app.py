...
def edit(id):
    ...
    if request.method == "GET":
        ...
        word = request.form["search"]
        search_list = []
        for flash_card in user.cards:
            card = Flashcard.objects(id=flash_card.id).first()
            if word == card.word:
                search_list.append(card)
        return render_template("search result id.html", search_list=search_list, id=id)
...
def delete():
    ...
    if card is not None:
        ...
        user.update(set__cards = flash_Card_list)
        card.delete()
        # print(card.objects)
        return render_template("edit.html", id=id, flashcard_list=user.cards)
    elif card is None:
